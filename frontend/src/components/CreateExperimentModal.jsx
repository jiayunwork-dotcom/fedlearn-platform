...
import {
  ...
  Box, Chip, Alert, Accordion, AccordionSummary, AccordionDetails
} from '@mui/material'
...
export default function CreateExperimentModal({ open, onClose }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const startRes = await experimentApi.start(expId)
      onClose()
  ...
  return (
    ...
    >
      <DialogTitle sx={{ fontWeight: 700 }}>创建联邦学习实验</DialogTitle>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} sm={4}>
            <TextField
              ...
              onChange={(e) => handleChange('num_rounds', parseInt(e.target.value) || 20)}
            />
          </Grid>
          <Grid item xs={12} sm={4}>
            <TextField
              ...
              onChange={(e) => handleChange('local_epochs', parseInt(e.target.value) || 5)}
            />
          </Grid>
          ...
          <Grid item xs={12} sm={4}>
            <TextField
              ...
              onChange={(e) => handleChange('batch_size', parseInt(e.target.value) || 64)}
            />
          </Grid>
          <Grid item xs={12} sm={4}>
            <TextField
              ...
              onChange={(e) => handleChange('learning_rate', parseFloat(e.target.value) || 0.01)}
            />
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            <Slider
              value={config.client_sample_rate * 100}
              min={10}
              max={100}
              step={5}
              onChange={(_, v) => handleChange('client_sample_rate', v / 100)}
            />
          </Grid>
          ...
          {config.algorithm === 'fedprox' && (
            <Grid item xs={12}>
              ...
              <Slider
                ...
                max={1}
                step={0.001}
                onChange={(_, v) => handleChange('fedprox_mu', v)}
              />
            </Grid>
          )}
          ...
          <Grid item xs={12} sm={6}>
            <FormControlLabel
              control={
                <Switch
                  checked={config.secure_aggregation}
                  onChange={(e) => handleChange('secure_aggregation', e.target.checked)}
                />
              }
              label={
                <Box>
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => handleChange('differential_privacy', e.target.checked)}
                />
              }
              label={
                <Box>
            ...
            <>
              ...
              <Grid item xs={12} sm={6}>
                <Box>
                  ...
                  <Slider
                    value={config.secagg_dropout_rate * 100}
                    min={0}
                    max={50}
                    step={1}
                    onChange={(_, v) => handleChange('secagg_dropout_rate', v / 100)}
                  />
                </Box>
              </Grid>
            </>
              ...
              <Grid item xs={12} sm={4}>
                <Box>
                  ...
                  <Slider
                    ...
                    max={5}
                    step={0.1}
                    onChange={(_, v) => handleChange('dp_clip_norm', v)}
                  />
                </Box>
              </Grid>
              <Grid item xs={12} sm={4}>
                <Box>
                  ...
                  <Slider
                    value={config.dp_noise_multiplier}
                    min={0.1}
                    max={5}
                    step={0.1}
                    onChange={(_, v) => handleChange('dp_noise_multiplier', v)}
                  />
                </Box>
              </Grid>
              <Grid item xs={12} sm={4}>
                <TextField
                  ...
                  type="number"
                  label="目标隐私预算 ε"
                  value={config.dp_target_epsilon}
                  InputProps={{ inputProps: { min: 0.1, max: 100, step: 0.1 } }}
                  onChange={(e) => handleChange('dp_target_epsilon', parseFloat(e.target.value) || 5)}
                />
              </Grid>
            </>
          )}
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              ...
              <Select
                value={config.non_iid_mode}
                label="非IID模式"
                onChange={(e) => handleChange('non_iid_mode', e.target.value)}
              >
          ...
          <Grid item xs={12} sm={6}>
            <Box>
              ...
              <Slider
                value={config.non_iid_alpha}
                min={0.01}
                max={5}
                step={0.01}
                disabled={config.non_iid_mode === 'iid'}
                onChange={(_, v) => handleChange('non_iid_alpha', v)}
              />
              {config.non_iid_mode !== 'iid' && (
                <Typography variant="caption" color="text.secondary">
                  α越小偏斜越严重 (0.01=极端偏斜, 5=近似IID)
                </Typography>
              )}
            </Box>
          </Grid>
            ...
            <>
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={config.byzantine_attack}
                    label="攻击类型"
                    onChange={(e) => handleChange('byzantine_attack', e.target.value)}
                  >
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth>
                  ...
                  <Select
                    value={config.robust_aggregation}
                    label="鲁棒聚合算法"
                    onChange={(e) => handleChange('robust_aggregation', e.target.value)}
                  >
